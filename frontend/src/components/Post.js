...
export default function Post({post,setPost,ai,setAi,heading,userId}) {
  const insertEmbedPostItem = {
    ...
    execute: async (editor)=>{
      ...
      await axios.post("/post/getById",{postId:postId,userId:userId
    }).then((res) => {
      ...
      await axios.put("/post/update",{id:postId,parentId:post._id}).then((res) => {}).catch((err) => {console.log(err);});
    },
    aliases: ["embed", "ed"],
    group: "Other",
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      axios.put("/post/update",{id:post._id,content:JSON.stringify(editor.topLevelBlocks),title:heading?heading:"Untitled"}).then((res) => {setPost({_id:post._id, content:JSON.stringify(editor.topLevelBlocks)}) }).catch((err) => {console.log(err);});
  ...
  useEffect(() => {
    if(ai)
    {
      axios.post("/cia/postHelper",{botname:ai,content:traverse(editor.topLevelBlocks)}).then((res)=>{
        // console.log(res); 
        setAi();
        editor.insertBlocks(
