...
export default function Notes({userId}) {
  ...
  useEffect(() => {
    axios.post("/post/getByUserId",{
        userId:userId
    }).then((res) => {
      setPosts(res.data.reverse());
      if(searchParams.get("new")){
        setSearchParams({});
        axios.post("/post/create",{userId:userId}).then((res) => {
    ...
    axios.post("/community/getByUserId",{userId:userId}).then((res) => {
  ...
  const handlePublish= async()=>{
    closeModal();
    axios.post("/community/publish",{postId:post._id,communityId:community._id}).then((res) => {
