...
export default function PostPage({userId}) {
    ...
    useEffect(() => {
        axios.post("/post/getById",{
            postId:id ,userId:userId
        }).then((res) => {
            res.data.isPublished=true;
            setPost(res.data);
            axios.post("/user/getUser",{userId:res.data.userId}).then((res) => {
                setOwner(res.data);
            }).catch((err) => {console.log(err);});
        }).catch((err) => {
            console.log(err);
        });
        axios.post("/user/getUser",{userId:userId}).then((res) => {
    ...
    const handleNewReply= async(name)=>{
        axios.post("/cia/postHelper",{botname:name,content:traverse(JSON.parse(post.content))}).then((res)=>{
        console.log(res); 
        setReply(res.data);
      })
      .catch((err)=>console.log(err));
